# copied from sample.py, recitified the sample weights
...
folders = {'arxiv': 0.0038885832078429925, 'cn_baike': 0.013171063389765087, 'cn_book': 6.913036813943098e-06, 'cn_weixin': 0.11492975550956504, 'cn_zhihu': 0.06456880079775063, 'code_starcoder': 0.16964341743831857, 'en_book': 0.0005333269641220821, 'en_stackexchange': 0.07731814128971792, 'wanjuan': 0.4785982974469239, 'wiki': 0.07734170091917984}
...
def get_files_dict(folder_path):
    ...
    for root, dirs, files in os.walk(folder_path):
        for dir in dirs:
            ...
            files_dict[dir] = [os.path.join(dir_path, file) for file in os.listdir(dir_path) if os.path.isfile(os.path.join(dir_path, file))]
    return files_dict

folder_to_files = get_files_dict(args.tokenized_dir)
...
"""
遍历所有文件夹
    将文件夹中的文件一分为2，一半用于eval，一半用于train/prune
    eval: 根据传入的eval_seq参数，确定要从每个文件中sample多少行用于eval
    prune/train: 
        根据for_prune/for_ft和folder的比例，计算每个folder需要sample多少数据
        遍历每个文件
            根据folder的文件数量和folder需要sample的数据确定每个file需要采样多少数据用于prune/train
            采样足够的数据后退出
"""
...
# seq_1b is the number of sequences for 1B data, therefore if we need 100B training data, the for_ft should be set to 100
